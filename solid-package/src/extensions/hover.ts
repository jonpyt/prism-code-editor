...
const editorHoverDescriptions = (
	...
	return editor => {
		const [show, hide, tooltip, container] = createHoverTooltip(editor, callback, options)
		...
		onCleanup(
			addTextareaListener(editor, "mouseleave", e => {
				if (!tooltip.contains(e.relatedTarget as Element)) hide()
			}),
		)

		onCleanup(addPointerListener(editor, "pointermove", handler, options.filter))
		onCleanup(addPointerListener(editor, "pointerdown", handler, options.filter))
