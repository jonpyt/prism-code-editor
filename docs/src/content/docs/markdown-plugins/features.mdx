---
title: Features
description: Learn which features are offered by the rehype plugin, markdown-it plugin, and marked extension.
---
...
You can provide a custom render function to wrap the default render function or completely override it. The example below wraps the output to add a title with a file name for code blocks. Add a `title` meta property to the code block, and you should see a `<figcaption>{:html}` element with the specified title.
...
In addition to all features described in the [code blocks guide](/guides/code-blocks), code blocks support adding extra classes to lines with property values wrapped in `{}`. This is intended for line highlighting.
...
```css editor="false" del={2,3} ins={7,8} highlight={12,13}
.pce-line.del {
	background: #ff11002e;
