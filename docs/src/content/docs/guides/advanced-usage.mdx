...
The 3rd argument onward to `createEditor(){:js}` are the extensions you want the editor to be initialized with, but you can also add more extensions later with the [`editor.addExtensions(){:js}`](/api-reference/index/i-prismeditor#addextensions) method. The example shown uses a dynamic `import(){:js}` to load most extensions only when the editor is focused. This enables code splitting for even faster load times. Note that the extensions might not have loaded before the user starts typing in this case. In your application, there might be better events to listen to for starting the import.
...
import { editorCommands, defaultKeymap, editHistory } from "prism-code-editor/commands"
