...
- [`editorCommands(){:js}`](/api-reference/commands/f-editorcommands/) adds many commands expected from an editor.
...
- [`customCursor(){:js}`](/api-reference/cursor/f-customcursor/) overrides the browser's cursor and allows changing the blinking animation and animating the position.
...
- [`highlightSelectionMatches(){:js}`](/api-reference/search/f-highlightselectionmatches/) highlights all instances of the user's selected text.

Once added to an editor, many extensions can be accessed from the [`editor.extensions{:js}`](/api-reference/index/i-prismeditor/#extensions) property. Later guides will show some of the more sophisticated extensions in detail.
...
It's not possible to remove extensions once they've been added to an editor, and most extensions don't allow changing their configuation options after creation. One solution is to replace the exisiting editor with a new editor with different extensions added. The example below shows how:
...
const replaceEditor = <T extends {} = {}>(
	editor: PrismEditor<T>,
	newOptions: Partial<EditorOptions<T> & T>,
	...extensions: EditorExtension<T>[]
): PrismEditor<T> => {
	const parent = editor.container.parentNode
	const history = editor.extensions.history
	...
	const newEditor = createEditor(
		parent,
		{ ...editor.options, value: editor.value, ...newOptions },
		...(history ? [history] : []),
		...extensions,
	)

	if (focused) newEditor.textarea.focus()
	setSelection(newEditor, ...selection)
	return newEditor
}
```

Here, the history extension (if present) is moved to the new editor which transfers the history.
