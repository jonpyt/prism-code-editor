...
If you're using the [React rewrite](https://github.com/jonpyt/prism-code-editor/tree/main/react-package) or [SolidJS rewrite](https://github.com/jonpyt/prism-code-editor/tree/main/solid-package), then the package for that rewrite should be installed instead. See the [usage with frameworks guide](/guides/usage-with-frameworks/) for more info.
...
The second argument (type [`EditorOptions{:js}`](/api-reference/index/types/editoroptions)) to the setups allows some basic configuration of the created editor. Options can be altered after creation with the [`editor.setOptions(){:js}`](/api-reference/index/i-prismeditor/#setoptions) method. Below is a list of the allowed properties.
