...
The [`defaultKeymap`](/api-reference/commands/v-defaultkeymap) binds the Tab key to indenting code. Users can at any time toggle tab capturing with `Ctrl+M` or `⌃ ⇧ M` (Mac) whenever an editor is focused, but if a keyboard navigator is unaware of this, they're effectively trapped in the editor.
