...
This library can also render static code blocks. The code blocks require no JavaScript to work and support some features that editors don't. Every code block on this page has been generated with this library.
...
The only argument to `renderCodeBlock(){:js}` is a [`CodeBlockOptions{:js}`](/api-reference/ssr/t-codeblockoptions) object. These options include a few properties not present in `EditorOptions{:js}` such as `lineNumberStart`, `preserveIndent`, `guideIndents`, and `tokenizeCallback`.
...
`lineNumberStart` can be used to configure the line number of the first line.
...
Just like editors, code blocks support modifying tokens through the `tokenizeCallback` option. See the [SSR guide](/guides/server-side-rendering#modifying-tokens) for more info.
...
`forEachCodeBlock(){:js}` can be used to add a copy button with [`addCopyButton(){:js}`](/api-reference/code-blocks/f-addcopybutton).
...
The second parameter to `addCopyButton(){:js}` can be used to customize the copied text. If you're using [line highlighting](/markdown-plugins/features#line-highlighting) with one of the markdown plugins for example, you may want to use [`omitLines(){:js}`](/api-reference/code-blocks/f-omitlines) to remove lines marked as deleted from the copied text.

```js editor="false"
const codeBlocks = forEachCodeBlock(document, codeBlock => {
	addCopyButton(codeBlock, omitLines(".del"))
})
```
