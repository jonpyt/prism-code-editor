...
- `prism-code-editor/cursor.css`: Styles for [`customCursor(){:js}`](/api-reference/cursor/f-customcursor).
...
.prism-code-editor {
	...
	--pce-scrollbar: 210, 10%, 40%;
}
```

You can change the color of the scrollbar thumb using the custom property `--pce-scrollbar{:css}`. Different alpha values will be set based on the state of the scrollbar thumb.
...
- `--pce-bg{:css}`: Background for the editor. Set by all themes.
- `--pce-cursor{:css}`: Color of the editor's cursor. Set by all themes.
...
- `--pce-line-number{:css}`: Text color for the line numbers. Set by all themes.
- `--pce-bg-highlight{:css}`: Background for the line with the cursor.
- `--pce-border-highlight{:css}`: Border for the line with the cursor.
- `--pce-selection-match{:css}`: Background color of selection matches. Set by all themes.
- `--pce-scrollbar{:css}`: See [scrollbar styling](#scrollbar-styling). Set by all themes.
- `--pce-guide-indent{:css}`: Indentation guide color. Set by all themes.
...
- `--pce-bg-fold{:css}`: Color of the chevrons used to fold code. Defaults to `#777{:css}`.
- `--pce-widget-bg{:css}`: Background for editor widgets. Set by all themes.
- `--pce-widget-color{:css}`: Text color for editor widgets. Set by all themes.
- `--pce-widget-border{:css}`: Border color for editor widgets. Set by all themes.
- `--pce-widget-bg-input{:css}`: Background for input elements. Set by all themes.
- `--pce-widget-bg-active{:css}`: Background for selected widget buttons. Set by all themes.
- `--pce-widget-bg-hover{:css}`: Background for hovered widget buttons. Set by all themes.
- `--pce-widget-color-active{:css}`: Text color for selected widget buttons. Set by all themes.
- `--pce-widget-focus{:css}`: Focus ring color for widgets. Set by all themes.
- `--pce-widget-error{:css}`: Border color for invalid input elements. Set by all themes.
- `--pce-widget-bg-error{:css}`: Background for the error message popup. Set by all themes.
- `--pce-widget-color-options{:css}`: Text color used by the bottom row in the search widget. Set by all themes.
- `--pce-search-match{:css}`: Background for search matches. Set by all themes.

<Aside type="note">
	Most variable names were recently changed in the 5.0.0 release. Check the [release notes](https://github.com/jonpyt/prism-code-editor/releases/tag/5.0.0) for which names were changed.
</Aside>
